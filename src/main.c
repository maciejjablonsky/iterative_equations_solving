...
void exercise_A(void) {
        ...
        printf("Matrix A\n");
        matrix__print_compact(A);
        printf("Vector b transposed\n");
        matrix__print_compact(b_transposed);
...
void exercise_B() {
        ...
        int w = 20;
        printf("%*s\t%*s\t%*s\t%*s\t%*s\n", w, " ", w, "matrix size [n]", w, "iterations [n]",
               w, "whole time [s]", w, "hot loop time [s]");
...
void exercise_D(void) {
        ...
        printf("%*s\t%*s\t%*s\t%*s\t%*s\n", w, " ", w, "matrix size [n]",
               w, "whole time [s]", w, "hot loop time [s]", w, "solution norm");
        printf("%*s\t%*u\t%*.4lf\t%*.4lf\t%*.10Le\n", w, "Jacobi", w, lu.matrix_size,
               w, perf_whole_time(&lu), w, lu.hot_loop_time_seconds, w, lu.solution_norm);
}
...
void exercise_E() {
        ...
        for (int i = 0; i < N_len; ++i) {
                ...
                printf("%*.4lf\t", w, lu[i].hot_loop_time_seconds);
